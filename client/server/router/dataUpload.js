...
let disk = ''
const setDisk = (value)=> { disk = value.body.title }
...
const makeFolder = (dir) => { if(!fs.existsSync(dir)) { fs.mkdirSync(dir) } }

//게임데이터 업로드 요청
router.post('/postInfo',function(요청, 응답){
  let 게임데이터 = {
      title : 요청.body.title,
      genre : 요청.body.genre,
...
//이미지 저장요청
router.post('/postImg', upload, (요청, 응답)=>{ 
  ...
  let gameImage = {
    ...
    image_main : 요청.files.image_main[0].filename,
    image_background : 요청.files.image_background[0].filename,
    youtube_url : 요청.body.youtube_url 
  }
  db.collection('game_data_image').insertOne(gameImage, (에러, 결과)=>{
    if(!결과) console.log(에러)
  })
  db.collection('game_data').updateOne({ _id : ObjectId(요청.body.parent) }, { $set : { image_main : 요청.files.image_main[0].filename }},(에러, 결과)=>{
    if(!결과) console.log(에러)
  })
  
  응답.status(200).send({ message : '이미지 저장성공!' })
  console.log(요청.files.image_main[0].filename)
  console.log(요청.files.image_background[0].filename)
})
